...
import {
    saveGame,
    updateGameDetailsApiCall,
} from "../../helpers/updateGameDetails";
...
const ChessBoard: React.FC<ChessBoardProps> = ({ roomId }) => {
    ...
    const [userPieceColor, setUserPieceColor] =
        useState<ChessPieceColor>("white");
    ...
    useEffect(() => {
        ...
        socket.on(
            socketListenEvents.CHESS_GAME_OVER,
            (data: GameOverInterface) => {
                setGameOver(data);
            }
        );
        ...
        socket.on(
            socketListenEvents.OPPONENT_CASTLED,
            (cellsClicked: ClickedCellsType) => {
                let tempBoard = board.map((b) => b);
                game.movePiece(tempBoard, cellsClicked);
                setBoard(tempBoard);
        ...
        socket.on(
            socketListenEvents.CHESS_PLAYER_2_JOINED,
            (data: { users: string[] }) => {
                const p2Name = data.users.filter(
    ...
    const handlePawnPromotion = (pieceName: ChessPieceName) => {
        ...
        socket.emit(socketEmitEvents.PAWN_PROMOTED, {
            pieceName,
            cellsClicked,
        });
    ...
    const showMoves = (row: number, col: number) => {
        if (!gameOver.gameOver) {
            let tempBoard = board.map((b) => b);
            let returnedValue = game.showValidMoves(
                userPieceColor,
                tempBoard,
                row,
                col
            ...
            if (returnedValue) {
                if ("cellsClicked" in returnedValue) {
                    const { cellsClicked, castlingDone, pawnPromoted } =
                        returnedValue;
                    ...
                    if (!castlingDone && !pawnPromoted) {
                        socket.emit(
                            socketEmitEvents.USER_PLAYED_A_MOVE,
                            cellsClicked
                        );
                    }
                    if (castlingDone) {
                    ...
                    if (pawnPromoted) {
                        ...
                        const pawn = tempBoard[cellsClicked.rows[1]][
                            cellsClicked.cols[1]
                        ] as Pawn;
            ...
            if (game.isGameOver(board)) {
                ...
                socket.emit(
                    socketEmitEvents.CHESS_GAME_OVER,
                    newGameOverObject
                );
        ...
        <div style={{ margin: windowSize[0] < 910 ? "2rem 0" : "" }}>
            ...
            <div
                ...
                style={{
                    ...
                    objectFit: "contain",
                    backgroundSize: `${CELL_SIZE * board.length}px, ${
                        CELL_SIZE * board.length
                    }px`,
                }}
            >
                {gameOver.gameOver && (
                ...
                {showPawnPromotionDialog.show && (
                    <PawnPromotionDialog
                        pawnColor={
                            showPawnPromotionDialog.pawnColor as ChessPieceColor
                        }
