...
class King extends ChessPiece {
    ...
    notAllowKingToMoveToAttackedCell = (kingParameters: KingParametersType) => {
        ...
        const { cellsUnderAttackByWhite, cellsUnderAttackByBlack } =
            kingParameters;
    ...
    isRookPresent = (board: ChessBoardType, rookColor: ChessPieceColor) => {
        ...
        if (!(leftRook instanceof Rook) && !(rightRook instanceof Rook))
            return false;
    ...
    addCastlingMoves = (
        ...
        if (this.color === "white") {
            ...
            const { leftBlocked, rightBlocked } =
                this.isPathBetweenRookAndKingBlocked(board, this.color);
    ...
    handleKingInCheckByQueen = (
        genericKingParams: GenericKingParametersType
    ) => {
            ...
            if (column > -1 && column < 8) {
                rowsArray[i].forEach((rowAdder) => {
                    if (this.row + rowAdder > -1 && this.row + rowAdder < 8) {
                        ...
                        if (piece === 0) {
                            this.moves[getStr(this.row + rowAdder, column)] =
                                "valid";
                        } else if (
                            ...
                            this.protectingMoves[
                                getStr(this.row + rowAdder, column)
                            ] = "protecting";
                        }
                    }
                });
